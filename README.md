# Smart Image MCP - Multi-Provider AI Image Generation Server for Claude Desktop & MCP Clients
...
## What is Smart Image MCP?

Smart Image MCP is a **Model Context Protocol (MCP) server** that provides unified access to multiple AI image generation providers. Whether you're using **Claude Desktop**, **Kiro IDE**, or any MCP-compatible client, this server lets you generate and transform images using:
...
| Style | Model | Output |
|-------|-------|--------|
| **Technical Diagram** | OpenAI `gpt-image-1.5` | ![OpenAI Technical](images/smart-image-mcp-architecture.png) |
| **Technical Diagram** | Google `gemini-2.5-flash-image` | ![Gemini Technical](images/smart-image-mcp-architecture-gemini.png) |
| **Technical Diagram** | AWS `amazon.nova-canvas-v1:0` | ![Nova Technical](images/smart-image-mcp-architecture-nova.png) |
| **3D Clay Art** | Google `gemini-2.5-flash-image` | ![Gemini Clay](images/smart-image-mcp-architecture-clay-gemini.png) |
| **3D Clay Art** | OpenAI `gpt-image-1.5` | ![OpenAI Clay](images/smart-image-mcp-architecture-clay-openai.png) |
| **3D Clay Art** | AWS `amazon.nova-canvas-v1:0` | ![Nova Clay](images/smart-image-mcp-architecture-clay-nova.png) |
...
```
Technical architecture diagram of a Smart Image MCP Server system. The diagram shows:

Top layer: MCP Client (Claude Desktop, Kiro IDE) connecting via Model Context Protocol

Middle layer: Smart Image MCP Server (FastMCP) with three main components:
...
`mcp-server` `image-generation` `ai-images` `aws-bedrock` `openai` `google-gemini` `claude-desktop` `dall-e` `imagen` `nova-canvas` `python` `fastmcp` `model-context-protocol` `ai-art` `text-to-image` `image-transformation`
